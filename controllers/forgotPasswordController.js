...
class ForgotPasswordController {
    ...
    static forgotPassword = async (req, res) => {
        ...
        try {
          ...
          res.status(200).send({ "status": "success", "message": "OTP sent to your email. Verify it to complete login." });
    ...
    static verifyForgotPasswordOtp = async (req, res) => {
      ...
      if (!otp) {
        return res.status(400).json({ "status": "failed", "message": "Email or OTP not provided" });
      }
      ...
      try {
        ...
        const accessToken = jwt.sign({ userID: user._id, email: user.email }, process.env.JWT_SECRET_KEY, { expiresIn: '10m' });
    ...
    static setNewPassword = async (req, res) => {
      ...
      if (!accessToken) {
        return res.status(401).json({ "status": "failed", "message": "Unauthorized: Access token is required" });
      }
